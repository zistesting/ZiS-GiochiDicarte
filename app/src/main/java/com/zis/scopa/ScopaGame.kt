...
class ScopaGame {
    ...
    /**
     * Valore di primiera di un mucchio, con la regola tradizionale: **chi non ha preso
     * nemmeno una carta di un seme non concorre alla primiera**, e vale zero.
     *
     * Prima i semi mancanti contavano zero e si sommava il resto, quindi un giocatore con
     * carte in tre semi poteva vincere il punto lo stesso. Succede di rado - serve che
     * l'avversario si sia portato via tutte e dieci le carte di un seme - ma quando succede
     * il punto andava a chi per regola non poteva prenderlo.
     *
     * Se nessuno dei due e' in gioco valgono zero entrambi, e il confronto stretto in
     * scoreOf fa in modo che il punto non venga assegnato a nessuno: e' la stessa cosa che
     * accade in caso di parita', ed e' quello che vuole la regola.
     */
    private fun primieraValue(cards: List<Card>): Int {
        var total = 0
        for (s in 0..3) {
            total += cards.filter { it.suit == s }.maxOfOrNull { it.prime } ?: return 0
        }
        return total
    }
}
